...
timeoutInterval = 2
OOBFrame = 0
frameReqTime = [0 for i in range(30000)]
...
packetReqTime = [0 for i in range(30000)]
packetRTT = [0 for i in range(30000)]
packetReqServer = [5 for i in range(30000)]
packetRecvServer = [0 for i in range(30000)]
...
def init():
	...
	while frameBuffer[0] == ' ':
		if ((time.time() - packetReqTime[0]) > .2):
			requestPacket('0', random.randrange(4))
		receivePacket()
		...
		if frameBuffer[frame] == ' ' and packetReqTime[frame] == 0:
			if count < 8:
				...
				if count == 8:
					break				
	while (count < 8):
		framestorequest[count] = 31 + 8 - count
		count = count + 1
...
def receivePacket():
	...
	if readable:
		...
		if int(frameNumber) > (frameDisplayed + 31):
			...
			serverPerformanceInst[VM] = 1 / (serverInstAvgRTT[VM] + (packetsDroppedByServer[VM] * 0.1))
...
#		print "Factor:", congestionFactor
	...
	'''
for i in range(4):
	serverPerformance[i] = 1 / (serverAvgRTT[i] + (packetsDroppedByServer[i] * 0.02))
